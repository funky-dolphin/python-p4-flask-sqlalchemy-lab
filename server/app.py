...
def animal_by_id(id):
    ...
    <ul>ID: {animal.id}</ul>
    <ul>Name: {animal.name}\n</ul>
    <ul>Species: {animal.species}\n</ul>
    <ul>Zookeeper: {animal.zookeeper.name}\n</ul>
...
def zookeeper_by_id(id):
    ...
    if not animals:
        response_body += f'<ul>Zookeeper has no animals</ul>'
    else:
...
def enclosure_by_id(id):
    ...
    if not animals:
        response_body += f'<ul>Enclosure has no animals</ul>'
    else:
        for animal in animals:
            response_body += f'<ul>Animal: {animal.name}</ul>'
        response = make_response(response_body, 200)
